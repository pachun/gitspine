...
impl Repo {
    ...
    /// Load all commits with a flat graph (fast, no lane tracking).
    /// Returns the index of target_sha if found.
    pub fn load_all_flat(&mut self, target_sha: Sha) -> Option<usize> {
        if let Ok(git_repo) = Repository::open(&self.path) {
            let (commits, _) = Self::get_commits(&git_repo, None);
            self.graph = commit_graph::build_flat(&commits);
            self.commits = commits;
            self.commit_limit = None;
            self.has_more_commits = false;
            self.commits.iter().position(|c| c.sha == target_sha)
        } else {
            None
        }
    }

    /// Search full git history for commits matching the query.
    /// Uses git log --grep for message search, --author for author search.
    /// Returns the SHA of the first match, if any.
    pub fn search_full_history(&self, query: &str) -> Option<Sha> {
        ...
        // Try message search first (case-insensitive)
        ...
        if output.status.success() {
            let sha_str = String::from_utf8_lossy(&output.stdout);
            let sha_str = sha_str.trim();
            if !sha_str.is_empty() {
                if let Ok(oid) = git2::Oid::from_str(sha_str) {
                    return Some(oid);
                }
            }
        }
